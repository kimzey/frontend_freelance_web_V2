...
export default function EditProfile() {
  ...
  const [imgchage,setimgchage] = useState(0)
    ...
    const fetchuser = async () =>{
            await axios.get(`${import.meta.env.VITE_REACT_APP_API}api/user/${login_user}`)
            .then((response) => {
  ...
  const imageupload = async (e) => {
    ...
    await axios.put(`${import.meta.env.VITE_REACT_APP_API}api/user/updatimg/${login_user}`,{profile_img}, {headers: {'Content-Type': 'multipart/form-data',authorization:`Bearer ${getToken()}`}})
            .then((res) => {
                Swal.fire({
                    title: "แจ้งเตือน",
    ...
    const submit_edit = async () =>{
            await axios.put(`${import.meta.env.VITE_REACT_APP_API}api/user/${login_user}`,{name,password,ability,education,experience,link_html,name_pdf},{headers: {authorization:`Bearer ${getToken()}`}})
            .then(async (res)=>{
                console.log(res);
                console.log(name_pdf);
                await axios.put(`${import.meta.env.VITE_REACT_APP_API}api/user/updatpdf/${login_user}`,{name_pdf}, {headers: {'Content-Type': 'multipart/form-data',authorization:`Bearer ${getToken()}`}})
                .then((res) => {
                    console.log(res);
                    Swal.fire({
                        title: "แจ้งเตือน",
      ...
      <div className="EditProfile">
        ...
        <div className="EditprofilePcontainer">
          ...
          <div id="browsePic2">
            ...
            <img src={`${import.meta.env.VITE_REACT_APP_API}img_user/${profileimg||"avatar.png"}`} id="pic2" ></img>
            </div>
