...
export default function PostCompornent() {
    ...
    const fetchblogs = async () =>{
            await axios.get(`${import.meta.env.VITE_REACT_APP_API}api/posts/${param.category}/${page_number}`)
            .then((response) => {
    ...
    const submmit_form = async (e)=>{
        e.preventDefault();
        await axios.post(`${import.meta.env.VITE_REACT_APP_API}api/post/create`,{content,author,category},{headers: {authorization:`Bearer ${getToken()}`}})
        .then(async (response) => {
            const res_slut = response.data.slug
            await axios.put(`${import.meta.env.VITE_REACT_APP_API}api/post/updatimg/${res_slut}`,{content_img}, {headers: {'Content-Type': 'multipart/form-data',authorization:`Bearer ${getToken()}`}})
            .then((res) => {
                Swal.fire({
                    title: "แจ้งเตือน",
        ...
        <div className="post">
            ...
            <div className="post_container">
                ...
                {login_user && (
                    ...
                    {user && 
                    <> 
                        <img src={`${import.meta.env.VITE_REACT_APP_API}img_user/${user.name_img||"avatar.png"}`} id="profile_img" onClick={()=>navigate(`/profile/${user.username}`)}></img>
                        <h1>{user.username}</h1>
                    </>}
                    </div>
